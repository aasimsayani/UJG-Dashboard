...
import { fmtMonth, fmtDate, shortName, initials } from "../utils/helpers";
...
function UploadScreen({ groups, onFiles, onRemoveGroup }) {
  ...
  const onDrop = (e) => {
    ...
    const invalid = files.filter(f => !f.name.endsWith(".txt"));
    if (invalid.length) { setError("Please upload .txt files only (WhatsApp export format)."); return; }
    setError(null);
    onFiles(files);
  };
  ...
  return (
    ...
    >
      <div>
        ...
        <div style={{ ...S.uploadLogo, fontSize: "1.1rem", letterSpacing: "0.3em", opacity: 0.7, marginTop: "0.2rem" }}>
          COMMUNITY ANALYTICS
        </div>
      </div>
      <div style={S.divider} />
      ...
      <label style={{ ...S.dropArea, borderColor: dragging ? C.accent : C.border, cursor: "pointer" }}>
        ...
        <div style={{ fontSize: "1.05rem", color: C.textMuted, marginBottom: "0.4rem" }}>
      ...
      {error && (
        <div style={{ color: C.red, fontSize: "0.85rem", background: C.redSoft, padding: "0.5rem 1rem", borderRadius: 8, border: `1px solid ${C.red}44` }}>
          {error}
        </div>
      )}
...
function OverviewTab({ report, analytics, selMonth, trendData }) {
  ...
  return (
    <>
      ...
      {/* Contribution health row */}
      <div style={{ ...S.statGrid, gridTemplateColumns: "repeat(3,1fr)", marginBottom: "1.25rem" }}>
        {[
          [C.green,  "Active (All Time)",   report.dirActiveCount,  "sent msg in last 30 days"],
          [C.orange, "At-Risk",             report.dirAtRiskCount,  "30–60 days inactive"],
          [C.red,    "Silent (All Time)",   report.dirSilentCount,  "no messages or >60 days"],
        ].map(([ac, lbl, val, sub]) => (
          <div key={lbl} style={{ ...S.statCard, borderTop: `2px solid ${ac}` }}>
            <div style={S.statLabel}>{lbl}</div>
            <div style={{ ...S.statVal, color: ac }}>{val}</div>
...
function ConsistencyTab({ report, analytics }) {
  ...
  const sporadic = allMembers
    .filter(m => { const c = mc[m]; return c > 0 && c < minM; })
    .sort((a, b) => mc[b] - mc[a]);
...
function AllMembersTab({ analytics, selMonth }) {
  ...
  const sorted = [...allMembers].sort(
    (a, b) => (mms[selMonth]?.[b]?.messageCount || 0) - (mms[selMonth]?.[a]?.messageCount || 0)
  );
  ...
  return (
    <Panel title="All Members" count={`${allMembers.length} total`}>
      <div style={{ overflowX: "auto" }}>
        <table style={S.tbl}>
          <thead><tr>
            {["Member", "Store", "This Month", "Active Months", "Consistency", "All-Time Deals", "Status"].map(h => (
              <th style={S.th} key={h}>{h}</th>
            ))}
          </tr></thead>
          <tbody>
            {sorted.map(name => {
              ...
              const dirRec = memberDirectory?.get(name);
              const store  = dirRec?.storeName !== "Not Available" ? dirRec?.storeName : "—";
              return (
                <tr key={name}>
                  <td style={S.td}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.45rem" }}>
                  ...
                  <td style={{ ...S.td, fontSize: "0.8rem", color: store === "—" ? C.textFaint : C.textMuted }}>{store}</td>
...
export default function Dashboard() {
  ...
  const handleFiles = useCallback((files) => {
    for (const file of files) {
      if (!file.name.endsWith(".txt")) continue; // skip invalid
